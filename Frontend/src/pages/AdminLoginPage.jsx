...
const AdminLoginPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axiosInstance.post("/admins/login", adminInfo);
      console.log(response.data);

      if (response.data) {
        toast.success("Logged in Successfully");

        // Save admin data in context and localStorage
    ...
    <div className="min-h-screen bg-green-600 flex items-center justify-center">
      ...
      <div className="bg-white shadow-lg rounded-lg p-8 w-[90%] max-w-md">
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-lg font-semibold text-gray-800 mt-4">Admin</h2>
        </div>
